...
const SECRET_ADMIN = 'admion_sefdfs';
const SECRET_USER = 'clientSdf42';
...
const authenticateAdminJwt = (req, res, next) => {
  ...
  if (authHeader) {
    ...
    console.log(token);
    jwt.verify(token, SECRET_ADMIN, (err, user) => {
        console.log("in ");
       // console.log(user);
      if (err) {
        console.log(err);
...
const authenticateUserJwt = (req, res, next) => {
    ...
    if (authHeader) {
      const token = authHeader.split(' ')[1];
      jwt.verify(token, SECRET_USER, (err, user) => {
        if (err) {
          return res.sendStatus(403);
        }
        req.user = user;
...
app.post('/admin/signup', (req, res) => {
  ...
  if (admin) {
    ...
    const token = jwt.sign({ username, role: 'admin' }, SECRET_ADMIN, { expiresIn: '1h' });
...
app.post('/users/signup', (req, res) => {
  ...
  if (user) {
    ...
    const token = jwt.sign({ username, role: 'user' }, SECRET_USER, { expiresIn: '1h' });
